...
const FilterRule = ({ rule, onUpdate, onDelete, depth = 0 }: Props): React.ReactElement => {
  const [showWarning, setShowWarning] = useState(false);
  const [warningMessage, setWarningMessage] = useState("");

  const validOperators = getValidOperators(rule.column);

  const handleColumnChange = (newColumn: string): void => {
  ...
  const handleValueChange = (newValue: string | string[]): void => {
    onUpdate(rule.id, { value: newValue });
  };

  return (
    <div
      className={`border border-gray-200 rounded-lg p-4 bg-white ${depth > 0 ? "ml-6" : ""
        }`}
    >
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select
          ...
          className={`px-3 py-2 border rounded-md bg-white min-w-40 ${showWarning ? "border-yellow-500 bg-yellow-50" : "border-gray-300"
            }`}
        >
